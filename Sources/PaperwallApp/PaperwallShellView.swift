...
struct PaperwallShellView: View {
                ...
                if let previewURL {
                    ImmersiveWallpaperPreview(
                        ...
                        setWallpaper: {
                            ...
                            selectWallspace(previewURL) { result in
    ...
    private var libraryLoadingContent: some View {
        VStack(spacing: 12) {
            if wallspaceLibrary.isLoading {
                ...
                Button("Refresh", action: wallspaceLibrary.synchronize)
                    .buttonStyle(.plain)
    ...
    private func openLibraryPreview() {
        withAnimation(.smooth(duration: 0.3)) {
            section = .library
            previewURL = wallspaceLibrary.videos.first
        }
        if wallspaceLibrary.videos.isEmpty { wallspaceLibrary.synchronize() }
    }

    private func movePreview(by offset: Int) {
        let videos = wallspaceLibrary.videos
