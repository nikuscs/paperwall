...
struct WallspaceLibraryView: View {
    @ObservedObject var library: WallspaceLibraryModel
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if videos.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text(
                        library.isLoading
                            ? "Copying Wallspace videos into your Paperwall library."
                            : "Open wallpapers in Wallspace, then refresh this library."
                    )
                )
                ...
                HStack(spacing: 14) {
                    ZStack {
                        ...
                        if let selectedURL {
                            WallspacePlayerPreview(url: selectedURL)
...
@MainActor
private struct WallspacePlayerPreview: NSViewRepresentable {
    let url: URL
